const MEMORY_STYLE_ID = "minijuegos-memory-game";

const DIFFICULTIES = Object.freeze({
  8: { id: 8, label: "Normal", pairCount: 8, cols: 4 },
  12: { id: 12, label: "Desafio", pairCount: 12, cols: 6 }
});

const SYMBOLS = ["🐶", "🐱", "🦊", "🐻", "🐼", "🐨", "🐯", "🦁", "🐸", "🐵", "🐮", "🐷"];
const RESOLVE_DELAY_MS = 820;

const MEMORY_STYLES = String.raw`
.app-shell:not(.app-shell-home) .screen.game-screen-memory {
  width: min(1120px, 100%);
}

.memory-shell {
  width: min(100%, 940px);
  max-height: 100%;
  margin: 0 auto;
  display: flex;
  flex-direction: column;
  gap: 16px;
  perspective: 1200px;
}

/* SCOREBOARD / HUD - Brass & Mahogany Plaque */
.memory-hud {
  display: grid;
  gap: 12px;
  padding: 20px 24px;
  border-radius: 24px;
  border: 4px solid #3d2314; /* Mahogany outline */
  background: linear-gradient(135deg, #1c1917 0%, #0c0a09 100%); /* Premium charcoal slate dashboard */
  box-shadow:
    0 15px 30px rgba(0, 0, 0, 0.5),
    inset 0 0 15px rgba(0, 0, 0, 0.8),
    inset 0 1px 0 rgba(255, 255, 255, 0.1);
  position: relative;
}

.memory-hud::after {
  content: "";
  position: absolute;
  inset: 1px;
  border: 1px solid rgba(204, 164, 59, 0.15); /* Internal gold trim */
  border-radius: 20px;
  pointer-events: none;
}

.memory-hud-top {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}

.memory-hud-copy {
  display: grid;
  gap: 6px;
  min-width: 0;
}

.memory-mode-row {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.memory-mode-pill {
  display: inline-flex;
  align-items: center;
  min-height: 28px;
  padding: 0 12px;
  border-radius: 999px;
  background: rgba(204, 164, 59, 0.15);
  color: #cca43b; /* Casino Gold */
  border: 1px solid rgba(204, 164, 59, 0.3);
  font-size: 0.68rem;
  font-weight: 800;
  letter-spacing: 0.08em;
  text-transform: uppercase;
  text-shadow: 0 1px 2px rgba(0,0,0,0.5);
}

.memory-mode-pill.is-soft {
  background: rgba(74, 222, 128, 0.1);
  color: #4ade80; /* Emerald Accent */
  border-color: rgba(74, 222, 128, 0.2);
}

.memory-title {
  margin: 0;
  color: #ffffff;
  font-size: 1.2rem;
  font-family: 'Outfit', 'Inter', sans-serif;
  font-weight: 700;
  letter-spacing: -0.01em;
  text-shadow: 0 2px 4px rgba(0, 0, 0, 0.5);
}

.memory-note {
  margin: 0;
  color: #a8a29e; /* Stone warm gray */
  font-size: 0.9rem;
  transition: color 0.3s ease;
}

.memory-note.is-match {
  color: #4ade80; /* Emerald green glow */
  font-weight: 700;
  text-shadow: 0 0 10px rgba(74, 222, 128, 0.4);
}

.memory-note.is-miss {
  color: #f87171; /* Ruby red glow */
  font-weight: 700;
  text-shadow: 0 0 10px rgba(248, 113, 113, 0.4);
}

.memory-stats {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 12px;
}

.memory-stat {
  display: grid;
  gap: 4px;
  padding: 12px 14px;
  border-radius: 16px;
  border: 1px solid rgba(204, 164, 59, 0.25);
  background: rgba(255, 255, 255, 0.02);
  box-shadow: 
    inset 0 0 8px rgba(0, 0, 0, 0.4),
    inset 0 0 12px rgba(204, 164, 59, 0.05);
}

.memory-stat-label {
  color: #a8a29e;
  font-size: 0.65rem;
  font-weight: 800;
  letter-spacing: 0.08em;
  text-transform: uppercase;
}

.memory-stat-value {
  color: #cca43b; /* Glowing gold numbers */
  font-size: 1.25rem;
  font-weight: 800;
  font-family: 'Outfit', 'Inter', sans-serif;
  line-height: 1.1;
  text-shadow: 0 0 8px rgba(204, 164, 59, 0.35);
}

/* CASINO GAME TABLE - Wood Framed Green Felt */
.memory-board-frame {
  position: relative;
  overflow: hidden;
  padding: 24px;
  border-radius: 32px;
  border: 12px solid #3d2314; /* Mahogany frame */
  background: radial-gradient(circle at 50% 50%, #165b33 0%, #0d3c20 100%); /* Green felt base */
  box-shadow:
    0 30px 60px rgba(0, 0, 0, 0.65),
    inset 0 0 50px rgba(0, 0, 0, 0.6), /* Deep felt shadow */
    inset 0 0 0 2px rgba(204, 164, 59, 0.4); /* Inner gold rim */
}

/* Felt Micro-Texture */
.memory-board-frame::before {
  content: "";
  position: absolute;
  inset: 0;
  pointer-events: none;
  background-image: 
    radial-gradient(rgba(255, 255, 255, 0.04) 1px, transparent 0),
    radial-gradient(rgba(0, 0, 0, 0.1) 1.5px, transparent 0);
  background-size: 4px 4px, 6px 6px;
  background-position: 0 0, 2px 2px;
  opacity: 0.75;
}

.memory-board {
  position: relative;
  z-index: 1;
  display: grid;
  grid-template-columns: repeat(var(--memory-cols), minmax(0, 1fr));
  gap: 16px;
  width: min(100%, calc((100dvh - 300px) * (var(--memory-cols) / (var(--memory-rows) * 1.08))));
  margin: 0 auto;
}

/* PREMIUM 3D PLAYING CARDS */
.memory-card {
  position: relative;
  aspect-ratio: 1 / 1.08;
  padding: 0;
  border: 0;
  background: transparent;
  cursor: pointer;
  perspective: 1000px;
  isolation: isolate;
  border-radius: 18px;
}

.memory-card[disabled] {
  cursor: default;
}

.memory-card-inner {
  position: relative;
  display: block;
  width: 100%;
  height: 100%;
  transform-style: preserve-3d;
  transition: transform 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275);
  box-shadow: 0 6px 12px rgba(0, 0, 0, 0.4);
  border-radius: 18px;
}

.memory-card:not([disabled]):hover .memory-card-inner {
  transform: translateY(-8px) rotateX(10deg) rotateY(4deg) scale(1.03);
  box-shadow: 
    0 22px 30px rgba(0, 0, 0, 0.55),
    0 0 15px rgba(204, 164, 59, 0.2);
}

.memory-card.is-open .memory-card-inner {
  transform: rotateY(180deg);
}

.memory-card-face {
  position: absolute;
  inset: 0;
  display: grid;
  place-items: center;
  box-sizing: border-box;
  border-radius: 18px;
  backface-visibility: hidden;
  overflow: hidden;
}

/* CLOSED CARD - Gold embossed filigree royal back */
.memory-card-front {
  border: 2px solid #cca43b; /* Elegant gold border */
  background: 
    radial-gradient(circle at 50% 50%, #991b1b 15%, #580d0d 100%); /* Rich ruby velvet */
  box-shadow:
    inset 0 0 12px rgba(0, 0, 0, 0.8),
    inset 0 0 0 1px rgba(255, 255, 255, 0.15);
}

/* Detailed gold guilloche grid pattern */
.memory-card-front::before {
  content: "";
  position: absolute;
  inset: 6px;
  border: 1px solid rgba(204, 164, 59, 0.45);
  border-radius: 12px;
  background-image: 
    repeating-linear-gradient(45deg, rgba(204, 164, 59, 0.08) 0px, rgba(204, 164, 59, 0.08) 1px, transparent 1px, transparent 8px),
    repeating-linear-gradient(-45deg, rgba(204, 164, 59, 0.08) 0px, rgba(204, 164, 59, 0.08) 1px, transparent 1px, transparent 8px);
}

/* Decorative corner scrolls inside card */
.memory-card-front::after {
  content: "";
  position: absolute;
  inset: 10px;
  border: 1px dashed rgba(204, 164, 59, 0.25);
  border-radius: 8px;
  pointer-events: none;
}

/* OPEN CARD - Ivory playing card style */
.memory-card-back {
  transform: rotateY(180deg);
  border: 2px solid #e2e8f0;
  background: radial-gradient(circle at 30% 30%, #ffffff 0%, #f6f5f0 100%); /* Ivory paper */
  box-shadow:
    inset 0 0 14px rgba(0, 0, 0, 0.06),
    inset 0 0 0 1px rgba(255, 255, 255, 0.8);
}

.memory-card-back::before {
  content: "";
  position: absolute;
  inset: 6px;
  border: 1px solid rgba(204, 164, 59, 0.25); /* Gold filigree frame line */
  border-radius: 12px;
  pointer-events: none;
}

/* MATCHED STATE - Golden magical glow */
@keyframes cardMatchGlow {
  0% {
    box-shadow: 
      0 0 15px rgba(204, 164, 59, 0.4),
      inset 0 0 12px rgba(204, 164, 59, 0.2);
    border-color: rgba(204, 164, 59, 0.6);
  }
  100% {
    box-shadow: 
      0 0 35px rgba(204, 164, 59, 0.95),
      inset 0 0 12px rgba(204, 164, 59, 0.4);
    border-color: #cca43b;
  }
}

.memory-card.is-match .memory-card-back {
  border-color: #cca43b;
  box-shadow: 
    0 0 25px rgba(204, 164, 59, 0.6),
    inset 0 0 14px rgba(204, 164, 59, 0.2);
  animation: cardMatchGlow 1.2s ease-in-out infinite alternate;
}

/* Emojis floating Specular shine */
.memory-symbol {
  position: relative;
  z-index: 1;
  font-size: clamp(2.2rem, 5.5vw, 3.4rem);
  line-height: 1;
  filter: drop-shadow(0 3px 6px rgba(0,0,0,0.18));
  transform: translateY(-2px);
}

/* Gold center seal */
.memory-card-badge {
  position: relative;
  z-index: 2;
  color: #cca43b;
  font-size: 1.2rem;
  font-weight: 900;
  letter-spacing: 0.1em;
  text-transform: uppercase;
  text-shadow: 0 1px 2px rgba(0,0,0,0.4);
  width: 44px;
  height: 44px;
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  border: 1.5px solid rgba(204, 164, 59, 0.6);
  background: 
    radial-gradient(circle at 35% 35%, rgba(255,255,255,0.1) 0%, rgba(0,0,0,0.3) 100%),
    #7a1212;
  box-shadow: 
    0 4px 8px rgba(0, 0, 0, 0.35),
    inset 0 1px 0 rgba(255, 255, 255, 0.2);
}

/* INTERACTIVE ANIMATIONS */
@keyframes cardMatchFlash {
  0% {
    transform: rotateY(180deg) scale(1);
    filter: brightness(1);
  }
  40% {
    transform: rotateY(180deg) scale(1.15);
    filter: brightness(1.7) drop-shadow(0 0 20px rgba(204, 164, 59, 0.9));
  }
  100% {
    transform: rotateY(180deg) scale(1);
    filter: brightness(1);
  }
}

@keyframes cardMissShake {
  0%, 100% { transform: rotateY(180deg) translateX(0); }
  15%, 45%, 75% { transform: rotateY(180deg) translateX(-8px) rotateZ(-2deg); }
  30%, 60%, 90% { transform: rotateY(180deg) translateX(8px) rotateZ(2deg); }
}

.memory-card.is-open.is-match .memory-card-inner {
  animation: cardMatchFlash 0.65s cubic-bezier(0.25, 0.8, 0.25, 1) both;
}

.memory-card.is-open.is-miss .memory-card-inner {
  animation: cardMissShake 0.55s cubic-bezier(0.36, 0.07, 0.19, 0.97) both;
}

.memory-card:focus-visible {
  outline: none;
}

.memory-card:focus-visible .memory-card-inner {
  box-shadow:
    0 0 0 3px #cca43b,
    0 12px 20px rgba(0, 0, 0, 0.5);
}

/* RESPONSIVE DESIGN */
@media (max-width: 900px) {
  .memory-shell {
    width: min(100%, 760px);
    gap: 12px;
  }

  .memory-board-frame {
    padding: 16px;
    border-width: 8px;
  }

  .memory-board {
    gap: 12px;
  }
}

@media (max-width: 760px) {
  .app-shell:not(.app-shell-home) .screen.game-screen-memory {
    width: min(100%, calc(100vw - 10px));
  }

  .memory-shell {
    gap: 8px;
  }

  .memory-hud {
    padding: 14px 16px;
    gap: 8px;
    border-width: 3px;
  }

  .memory-title {
    font-size: 1rem;
  }

  .memory-note {
    font-size: 0.82rem;
  }

  .memory-stats {
    gap: 8px;
  }

  .memory-stat {
    padding: 8px 10px;
    border-radius: 12px;
  }

  .memory-stat-label {
    font-size: 0.58rem;
  }

  .memory-stat-value {
    font-size: 1rem;
  }

  .memory-board-frame {
    padding: 10px;
    border-width: 6px;
    border-radius: 22px;
  }

  .memory-board {
    grid-template-columns: repeat(4, minmax(0, 1fr));
    gap: 8px;
  }

  .memory-card-face {
    border-radius: 12px;
  }
  
  .memory-card-inner {
    border-radius: 12px;
  }

  .memory-symbol {
    font-size: clamp(1.8rem, 7vw, 2.4rem);
  }

  .memory-card-badge {
    font-size: 0.9rem;
    width: 32px;
    height: 32px;
  }
}
`;

function ensureMemoryStyles() {
  if (typeof document === "undefined") {
    return;
  }

  if (document.getElementById(MEMORY_STYLE_ID)) {
    return;
  }

  const style = document.createElement("style");
  style.id = MEMORY_STYLE_ID;
  style.textContent = MEMORY_STYLES;
  document.head.append(style);
}

function escapeHtml(value) {
  return String(value).replace(/[&<>\"']/g, (char) => {
    const map = {
      "&": "&amp;",
      "<": "&lt;",
      ">": "&gt;",
      '"': "&quot;",
      "'": "&#39;"
    };
    return map[char];
  });
}

function normalizePairs(value) {
  return Number(value) === 12 ? 12 : 8;
}

function difficultyMeta(value) {
  return DIFFICULTIES[normalizePairs(value)];
}

function shuffle(items) {
  const next = [...items];
  for (let index = next.length - 1; index > 0; index -= 1) {
    const swapIndex = Math.floor(Math.random() * (index + 1));
    [next[index], next[swapIndex]] = [next[swapIndex], next[index]];
  }
  return next;
}

function buildDeck(pairCount) {
  const symbols = SYMBOLS.slice(0, pairCount);
  const deck = symbols.flatMap((symbol, pairId) => ([
    { id: `pair-${pairId}-a`, pairId, symbol },
    { id: `pair-${pairId}-b`, pairId, symbol }
  ]));
  return shuffle(deck);
}

function matchedCount(state) {
  return state.matchedPairIds.length;
}

function totalPairs(state) {
  return normalizePairs(state.pairCount);
}

function buildNote(state) {
  if (state.status === "won") {
    return "Todas las parejas encontradas.";
  }

  if (state.status === "resolving") {
    return "Esas dos cartas no coinciden.";
  }

  if (state.lastAction === "match") {
    return "Pareja encontrada. Sigue asi.";
  }

  if (state.openIndexes.length === 1) {
    return "Busca la segunda carta.";
  }

  return "Destapa dos cartas y recuerda sus posiciones.";
}

function createState(pairCount) {
  return {
    pairCount,
    cards: buildDeck(pairCount),
    openIndexes: [],
    matchedPairIds: [],
    moveCount: 0,
    status: "playing",
    lastAction: "ready",
    resolveAtMs: null
  };
}

function buildStat(label, value) {
  return `
    <div class="memory-stat">
      <span class="memory-stat-label">${escapeHtml(label)}</span>
      <strong class="memory-stat-value">${escapeHtml(String(value))}</strong>
    </div>
  `;
}

function isMatched(state, index) {
  const card = state.cards[index];
  return state.matchedPairIds.includes(card.pairId);
}

function isOpen(state, index) {
  return state.openIndexes.includes(index) || isMatched(state, index);
}

function renderHud(state) {
  const meta = difficultyMeta(state.pairCount);
  const noteClass = `memory-note${state.lastAction === "match" ? " is-match" : ""}${state.status === "resolving" ? " is-miss" : ""}`;
  return `
    <section class="memory-hud">
      <div class="memory-hud-top">
        <div class="memory-hud-copy">
          <div class="memory-mode-row">
            <span class="memory-mode-pill">Parejas</span>
            <span class="memory-mode-pill is-soft">${escapeHtml(meta.label)}</span>
          </div>
          <h3 class="memory-title">Memoria visual en un solo tablero</h3>
          <p class="${noteClass}">${escapeHtml(buildNote(state))}</p>
        </div>
      </div>
      <div class="memory-stats">
        ${buildStat("Movimientos", state.moveCount)}
        ${buildStat("Parejas", `${matchedCount(state)}/${totalPairs(state)}`)}
        ${buildStat("Cartas", state.cards.length)}
      </div>
    </section>
  `;
}

function renderCard(state, card, index) {
  const open = isOpen(state, index);
  const matched = isMatched(state, index);
  const disabled = state.status === "won" || state.status === "resolving" || open;
  const isMiss = state.status === "resolving" && open && !matched;
  const className = `memory-card${open ? " is-open" : ""}${matched ? " is-match" : ""}${isMiss ? " is-miss" : ""}`;

  return `
    <button
      class="${className}"
      type="button"
      data-action="game-action"
      data-game-action="flip-card"
      data-card="${index}"
      aria-label="${open ? `Carta ${card.symbol}` : "Carta oculta"}"
      ${disabled ? "disabled" : ""}
    >
      <div class="memory-card-inner">
        <div class="memory-card-face memory-card-front">
          <span class="memory-card-badge">MG</span>
        </div>
        <div class="memory-card-face memory-card-back">
          <span class="memory-symbol">${escapeHtml(card.symbol)}</span>
        </div>
      </div>
    </button>
  `;
}

function renderBoardGrid(state) {
  const cols = difficultyMeta(state.pairCount).cols;
  return `
    <section class="memory-board-frame">
      <div
        class="memory-board"
        style="--memory-cols:${cols};--memory-rows:${state.cards.length / cols};"
        role="img"
        aria-label="${escapeHtml(`Tablero de ${totalPairs(state)} parejas. ${matchedCount(state)} resueltas.`)}"
      >
        ${state.cards.map((card, index) => renderCard(state, card, index)).join("")}
      </div>
    </section>
  `;
}

function flipCard(state, index, nowMs) {
  if (state.status === "resolving" || state.status === "won") {
    return { ok: true, state };
  }

  if (!Number.isInteger(index) || index < 0 || index >= state.cards.length) {
    return { ok: false, reason: "invalid" };
  }

  if (isMatched(state, index) || state.openIndexes.includes(index)) {
    return { ok: true, state };
  }

  const nextOpen = [...state.openIndexes, index];
  if (nextOpen.length === 1) {
    return {
      ok: true,
      state: {
        ...state,
        openIndexes: nextOpen,
        lastAction: "peek"
      }
    };
  }

  const [firstIndex, secondIndex] = nextOpen;
  const firstCard = state.cards[firstIndex];
  const secondCard = state.cards[secondIndex];
  const nextMoveCount = state.moveCount + 1;

  if (firstCard.pairId === secondCard.pairId) {
    const nextMatched = [...state.matchedPairIds, firstCard.pairId];
    const complete = nextMatched.length === totalPairs(state);
    return {
      ok: true,
      state: {
        ...state,
        openIndexes: [],
        matchedPairIds: nextMatched,
        moveCount: nextMoveCount,
        status: complete ? "won" : "playing",
        lastAction: "match",
        resolveAtMs: null
      }
    };
  }

  return {
    ok: true,
    state: {
      ...state,
      openIndexes: nextOpen,
      moveCount: nextMoveCount,
      status: "resolving",
      lastAction: "miss",
      resolveAtMs: Number(nowMs) + RESOLVE_DELAY_MS
    }
  };
}

function resolveMismatch(state, nowMs) {
  if (state.status !== "resolving") {
    return { ok: true, state };
  }

  if (!Number.isFinite(state.resolveAtMs) || Number(nowMs) < state.resolveAtMs) {
    return { ok: true, state };
  }

  return {
    ok: true,
    state: {
      ...state,
      openIndexes: [],
      status: "playing",
      lastAction: "reset-open",
      resolveAtMs: null
    }
  };
}

export const parejasGame = {
  id: "memory",
  name: "Parejas",
  subtitle: "1 jugador",
  tagline: "Memoria visual",
  minPlayers: 1,
  maxPlayers: 1,
  hidePlayerNames: true,
  hideGlobalTurnMessage: true,
  hideDefaultPlayerChips: true,
  rules: [
    { title: "Objetivo", text: "Descubre todas las parejas del tablero con el menor numero de movimientos posible." },
    { title: "Turno", text: "En cada intento destapas dos cartas. Si coinciden, se quedan visibles." },
    { title: "Si fallas", text: "Cuando dos cartas no coinciden se vuelven a cerrar tras una pausa breve." },
    { title: "Dificultad", text: "Puedes jugar con 8 parejas en tablero compacto o 12 parejas en formato amplio." }
  ],
  getDefaultOptions() {
    return {
      pairs: 8
    };
  },
  normalizeOptions(options = {}) {
    return {
      pairs: normalizePairs(options.pairs)
    };
  },
  renderConfigPanel({ options }) {
    const current = normalizePairs(options?.pairs);
    return `
      <div class="block">
        <h3 class="block-title">Dificultad</h3>
        <div class="player-count-row">
          ${Object.values(DIFFICULTIES).map((meta) => `
            <button
              class="pill ${current === meta.id ? "is-active" : ""}"
              data-action="set-game-option"
              data-option="pairs"
              data-value="${meta.id}"
              data-value-type="number"
            >
              ${escapeHtml(`${meta.pairCount} parejas · ${meta.label}`)}
            </button>
          `).join("")}
        </div>
      </div>
    `;
  },
  createInitialState({ options }) {
    return createState(normalizePairs(options?.pairs));
  },
  getTurnSlot() {
    return 0;
  },
  getResult(state) {
    return state.status === "won" ? { type: "win" } : null;
  },
  getTurnMessage({ state }) {
    return buildNote(state);
  },
  applyAction({ state, action }) {
    if (!action || typeof action.type !== "string") {
      return { ok: false, reason: "invalid" };
    }

    if (action.type === "resolve-mismatch") {
      return resolveMismatch(state, action.nowMs);
    }

    if (action.type === "flip-card") {
      return flipCard(state, Number(action.card), action.nowMs);
    }

    return { ok: false, reason: "invalid" };
  },
  renderCardIllustration() {
    return `
      <div class="game-illustration" aria-hidden="true">
        <svg class="game-illustration-svg" viewBox="0 0 160 94" preserveAspectRatio="xMidYMid meet" role="presentation">
          <defs>
            <linearGradient id="memoryCardFront" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color="#9fc2ff" />
              <stop offset="100%" stop-color="#6f9be4" />
            </linearGradient>
            <linearGradient id="memoryCardBack" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color="#fffdf9" />
              <stop offset="100%" stop-color="#efe8dc" />
            </linearGradient>
          </defs>
          <rect x="18" y="12" width="124" height="70" rx="22" fill="#eef4fb" stroke="#d0dbe6" />
          <g transform="translate(34 20)">
            <g transform="rotate(-9 22 27)">
              <rect x="0" y="0" width="44" height="54" rx="13" fill="url(#memoryCardFront)" stroke="#7499d9" />
              <path d="M12 16H32M22 8V46" stroke="rgba(255,255,255,0.34)" stroke-width="5" stroke-linecap="round" />
            </g>
            <g transform="translate(24 2)">
              <rect x="0" y="0" width="44" height="54" rx="13" fill="url(#memoryCardBack)" stroke="#d9cfc2" />
              <text x="22" y="34" text-anchor="middle" font-size="20">🦊</text>
            </g>
            <g transform="translate(50 6) rotate(8 22 27)">
              <rect x="0" y="0" width="44" height="54" rx="13" fill="url(#memoryCardBack)" stroke="#d9cfc2" />
              <text x="22" y="34" text-anchor="middle" font-size="20">🐻</text>
            </g>
          </g>
        </svg>
      </div>
    `;
  },
  renderBoard({ state }) {
    ensureMemoryStyles();
    return `
      <section class="memory-shell">
        ${renderHud(state)}
        ${renderBoardGrid(state)}
      </section>
    `;
  },
  formatResult({ state }) {
    return {
      title: "Parejas completadas",
      subtitle: `${state.moveCount} movimientos para resolver ${totalPairs(state)} parejas.`,
      iconText: "◎",
      iconClass: "win"
    };
  }
};
